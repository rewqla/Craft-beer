...
namespace Craft_beer_backend.Services.Implements
{
    ...
    public class CraftBeerService : ICraftBeerService
    {
        ...
        {
            _craftBeerRepository.Add(new CraftBeer { ABV=model.ABV, Description=model.Description, ImageUrl=model.ImageUrl, Name=model.Name, Price=model.Price, Volume=model.Volume, Rating=model.Raiting});
        }
        public void UpdateProduct(FullProductViewModel model)
        {
            _craftBeerRepository.Update(new CraftBeer { Id=model.Id, ABV = model.ABV, Description = model.Description, ImageUrl = model.ImageUrl, Name = model.Name, Price = model.Price, Volume = model.Volume, OrderItems=model.OrderItems, Rating = model.Raiting });
        }
